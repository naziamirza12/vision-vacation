...
(function () {
    const scroller = document.getElementById("destinationsScroller");
    if (!scroller) return;

    const wrap = scroller.closest(".destinations__sliderWrap");
    const btnPrev = wrap.querySelector(".destinations__nav--prev");
    const btnNext = wrap.querySelector(".destinations__nav--next");

    // helper: card width + gap
    function stepSize() {
        const card = scroller.querySelector(".destination-card");
        if (!card) return 260;
    ...
    let autoTimer = null;
    const AUTO_MS = 2200; // speed (adjust)
...
document.addEventListener("DOMContentLoaded", function () {
    const input = document.getElementById("guestSummary");
...
function changeGuest(type, delta) {
    if (type === "adult") {
        guestAdults = Math.max(1, guestAdults + delta);
        document.getElementById("adultCount").innerText = guestAdults;
    } else if (type === "child") {
        guestChildren = Math.max(0, guestChildren + delta);
        document.getElementById("childCount").innerText = guestChildren;
    }

    const total = guestAdults + guestChildren;
    document.getElementById("guestSummary").value =
...
function sendToWhatsApp() {
    const location = document.getElementById("txtKeyword")?.value?.trim() || "Not specified";
    const date = document.getElementById("txtCheckDate2")?.value?.trim() || "Not selected";
    const phone = document.getElementById("txtPhone")?.value?.trim() || "Not provided";
    const guests = document.getElementById("guestSummary")?.value?.trim() || "1 Guest";
    ...
    const url = "https://wa.me/918739056222?text=" + encodeURIComponent(message);

    // redirect (works even when popups blocked)
    window.location.href = url;
}


// interpage form whatsapp
...
var visitOffice = document.getElementById("visitOffice").checked ? "Yes" : "No";

var message = 
"*New Travel Inquiry*%0A%0A" +
"Name: " + name + "%0A" +
"Phone: " + phone + "%0A" +
"Travellers: " + travellers + "%0A" +
"Tour Type: " + tourType + "%0A" +
"Visit Office: " + visitOffice;

var url = "https://wa.me/918739056222?text=" + message;
